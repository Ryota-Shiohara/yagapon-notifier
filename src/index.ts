...
import { getDepartmentColor } from './design/departmentColor';
...
app.post('/notify', authMiddleware, async (req: Request, res: Response) => {
  ...
  try {
    ...
    const { id, title, description, remindTime, location, department } =
      req.body;
    ...
    if (channel && channel.isTextBased()) {
      // Discord Embed を作成
      const embed = new EmbedBuilder().setColor(getDepartmentColor(department)); // デフォルト色
      ...
      console.log(
        `通知をチャンネル ${NOTIFICATION_CHANNEL_ID} に送信しました。`
      );
      res.status(200).send({ success: true, message: 'Notification sent' });
    } else {
      console.error(
        `チャンネルID ${NOTIFICATION_CHANNEL_ID} が見つからないか、テキストチャンネルではありません。`
      );
      res.status(500).send({
        error: 'Notification channel not found or is not a text channel',
      });
    }
  } catch (error) {
